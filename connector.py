"""auto_efficiency_test.py
-----------------------------------------------------------------------
批量扫测电源 / 模块效率的示例脚本
-----------------------------------------------------------------------
• 功能
  1. 通过 PyVISA 同时控制直流电源、辅助电源、电子负载与数据采集仪
  2. 依次拉载多档电流，采集 Vin/Iin 与 Vout/Iout
  3. 计算系统效率与功率损耗并即时写入 Excel
  4. 支持自动生成带时间戳的文件名，断电亦不丢数据
...
ADDRESS: Dict[str, str] = {
    "LOAD": "ASRL7::INSTR",                           # 电子负载串口
    "VIN_PS": "USB0::0x1698::0x0837::002000002608::INSTR",  # Vin 直流电源
    "VCC_PS": "USB0::0x1AB1::0x0E11::DP8C193003485::INSTR", # 辅助电源
    "DAQ": "USB0::0x0957::0x2007::MY49029470::INSTR",       # Keysight DAQ
}
...
def main() -> None:
    ...
    try:
        ...
        inst.close_all()
        wb.close()
        print("[DONE] 测试完成。结果保存在 Excel 文件中。")
